  ...
  function visitCode (block) {
    if (block.t === 'CodeBlock' || block.t === 'Code') {
      ...
      for (let l of codeClasses) {
        ...
        codeClasses = codeClasses.filter((cls) => { return cls !== lang })
        break
      }
      ...
      let linenoStart = 1
      if (!inline) {
        for (let c of ['numberLines', 'number-lines']) {
          if (codeClasses.includes(c)) {
            lineno = true
            codeClasses = codeClasses.filter((cls) => { return cls != c })
          }
        }
      ...
      if (!inline) {
        if (lineno) {
          code = `<ol start=${linenoStart}><li>${code.split('\n').join('</li><li>')}</li></ol>`
        } else {
          code = code.split('\n').join('<br />')
        }
      }
      ...
      code = `<code class="${codeClasses.join(' ')}">${code}</code>`
      if (!inline) {
